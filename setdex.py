...
def replace_farfetchd(data):
    if isinstance(data, dict):
        keys_to_replace = [key for key in data.keys() if "Farfetch'D" in key]
        for key in keys_to_replace:
            new_key = key.replace("Farfetch'D", "Farfetch’d")
            data[new_key] = data.pop(key)
            replace_farfetchd(data[new_key])
        for key in data.keys():
            replace_farfetchd(data[key])
    elif isinstance(data, list):
        for index in range(len(data)):
            replace_farfetchd(data[index])
    elif isinstance(data, str) and "Farfetch'D" in data:
        data = data.replace("Farfetch'D", "Farfetch’d")
        return data

# Ersetzen von 'Farfetch'D' durch 'Farfetch’d' in den geladenen Daten
