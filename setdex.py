...
for trainer in data:
    ...
    final_party = []
    clean_party = [normalize_name(x[:-2]) for x in data[trainer].keys()]  # Apostrophe normalisieren
    ...
    for mon_name in final_party:
        ...
        dirty_mon_name_normalized = normalize_name(dirty_mon_name)

        # Fehlerbehandlung für nicht gefundene Schlüssel
        if dirty_mon_name_normalized not in data[trainer]:
            continue  # Überspringen, wenn der Name nicht gefunden wird

        mon_name_actual = mon_name.split(' (')[0]

        # Mapping von Hidden Power
        for move in data[trainer][dirty_mon_name_normalized]["moves"]:
            if "Hidden Power~" in move:
                code = move.split("~")[-1]
                if code in hidden_power_map:
                    # Ersetzen der Hidden Power mit dem korrekten Typ
                    data[trainer][dirty_mon_name_normalized]["moves"][
                        data[trainer][dirty_mon_name_normalized]["moves"].index(move)
        ...
        if "(" in mon_name:
            ...
            ] = data[trainer][dirty_mon_name_normalized]
        else:
            mon_store[mon_name_actual][
...
try:
    ...
    print(f"Fehler beim Schreiben der Datei 'setdex.json': {e}")

try:
    with open('party_order.json', 'w', encoding="utf8") as f:
        json.dump(party_order, f, indent='\t', separators=(',', ': '), ensure_ascii=False)
except Exception as e:
    print(f"Fehler beim Schreiben der Datei 'party_order.json': {e}")
