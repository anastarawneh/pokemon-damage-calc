...
# Funktion zur Normalisierung von Apostrophen
def normalize_name(name):
    return name.replace("’", "'").replace("’D", "'D").replace("'", "'")
...
for trainer in data:
    ...
    for mon_name in final_party:
        ...
        # Debugging: Alle verfügbaren Schlüssel ausgeben
        if dirty_mon_name_normalized not in data[trainer]:
            print(f"Fehler: '{dirty_mon_name_normalized}' nicht gefunden in Trainer '{trainer}'")
            print("Verfügbare Schlüssel:", list(data[trainer].keys()))
            continue  # Skip to avoid KeyError
